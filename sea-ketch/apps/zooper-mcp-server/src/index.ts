...
/**
 * @notice Builds and configures the Zooper MCP server with all registered tools.
 */
class ZooperMcpServer {
    private readonly _server: McpServer;

    public constructor() {
        this._server = new McpServer({ name: 'zooper-mcp-server', version: '0.1.0' });
        this._registerTools();
    }
...
/**
 * @notice Hosts a ZooperMcpServer over HTTP, routing POST /mcp requests to a fresh transport per call.
 */
class ZooperMcpHttpHost {
    private readonly _port: number;

    /**
     * @notice Constructs a ZooperMcpHttpHost on the given port.
    ...
    private async _handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
        ...
        try {
            ...
            const mcpServer = new ZooperMcpServer();
...
new ZooperMcpHttpHost(3000).start();
